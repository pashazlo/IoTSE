...
void ui_file_editor_handle_event(ui_event_t evt) {
    ...
    // 1. Модальный перехват клавиатурой
            ...
            if (ui_keyboard_was_confirmed()) {
                ...
                if (text && strlen(text) > 0) {
                    if (fm_text_edit_insert_text(s_editor.cursor_line, s_editor.cursor_column, text)) {
                        s_editor.cursor_column += (uint16_t)strlen(text);
                    }
                }
            }
            adjust_scroll();
    ...
    // 2. Обычная навигация по документу
    switch (evt) {
        case UI_EVT_UP:
        ...
        case UI_EVT_SELECT:
            // Открываем клавиатуру для ввода слова
            ui_keyboard_open("");
            break;

#ifdef UI_EVT_BACK
        case UI_EVT_BACK:
            // Удаление символа сзади без клавиатуры
            if (s_editor.cursor_column > 0) {
...
void ui_file_editor_draw(gfx_canvas_t *canvas) {
    ...
    int16_t y = 10;
    int16_t text_x_offset = 35; // Отступ под номера строк (001)
         ...
         line_idx++) 
    {
        ...
        char num_buf[8];
        snprintf(num_buf, sizeof(num_buf), "%03u", line_idx + 1);
        gfx_canvas_draw_str(canvas, 2, y + CHAR_HEIGHT - 2, num_buf, UI_FONT, 0x8410);

        // 2. Рисуем видимую часть текста (с учетом scroll_x)
        ...
        if (line_idx == s_editor.cursor_line) {
            ...
            if (rel_col >= 0 && rel_col < VISIBLE_COLS) {
                int16_t cursor_x = text_x_offset + (rel_col * CHAR_WIDTH);
                gfx_canvas_draw_line(canvas, cursor_x, y, cursor_x, y + CHAR_HEIGHT, 0x07E0); // Зеленый курсор
            }
        }
    ...
    // 4. Отрисовка клавиатуры ПОВЕРХ редактора при необходимости
    if (ui_keyboard_is_open()) {
